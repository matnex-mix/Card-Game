...
function AXECONTROLS($el_s) {
	...
	try {
		...
		if(typeof $el_s == "string") $el_s = document.querySelectorAll($el_s);

		if($el_s.length) {

			for( $i=0; $i<$el_s.length; $i++){
...
AXECONTROLS.prototype.restart = async function (){
...
AXECONTROLS.prototype.moment = function ($ct) {
	...
	$ct = $ct.split(",");
	$func = btoa(Date.now()+Math.random()).replace(/=/gi,"");
	eval($func+' = function(thes){ thes[$ct[0]](); }');

	if(!$ct[2]) {

		if( $XE.checkExtendMoment($ct[1].trim()) ) {
			$XE.addListener($ct[1].trim(), $func, this);
		}

		if(this.index == -1) {
			this.forEach(function($obj){
				eval('$obj.DOM.addEventListener($ct[1].trim(), function(){'+$func+'($XE.control(this));})');
			});
		} else {
			eval('this.DOM.addEventListener($ct[1].trim(), function(){'+$func+'($XE.control(this));})');
		}
		...
		if( $XE.checkExtendMoment($ct[1].trim()) ) {
			$XE.addListener($ct[1].trim(), $func, this, $els);
		}

		if($els.length > 1) {
			$els.forEach(function($obj){
				eval('$obj.addEventListener($ct[1].trim(), function(){'+$func+'($XE.control(this));})');
			});
		} else {
			eval('$els[0].addEventListener($ct[1].trim(), function(){'+$func+'($XE.control(this));})');
		}
...
function AXELISTENERS() {

}

AXELISTENERS.prototype.inview = function(){
	$XE.extendedMoments['inview'].forEach(function ($obj, i){
		$target = [];
		...
		if( $obj.target!==null ){
			...
			if( $obj.body.index!==-1 ) {
				$target.push($obj.body.DOM);
			} else {
				$target = $obj.body;
			}
		}

		$target.forEach(function ($obj2){
			if( $obj2.DOM ){
				$obj2=$obj2.DOM;
			}
			$obj2 = $obj2.getBoundingClientRect();
			...
			bool_inview_x = $obj2.left >= 0 && $obj2.right <= win_w;
			bool_inview_y = $obj2.top >= 0 && $obj2.bottom <= win_h;

			if( $obj2.height >= win_h ){
				bool_inview_y = $obj2.top >= 0 && $obj2.top <= 120;
			}

			if( $obj2.width >= win_w ){
				bool_inview_x = $obj2.left >= 0 && $obj2.left <= 120;
			}

			if( bool_inview_x && bool_inview_y ) {
				if( !$obj.data['inview'] ){
					$XE.extendedMoments['inview'][i].data['inview'] = true;

					eval($obj.do+'($obj.body)');
					return;
				}
			} else {
				$XE.extendedMoments['inview'][i].data['inview'] = false;
			}
		});
	});
};
...
var $XE = {
	...
	checkExtendMoment: function( $moment ) {
		if( typeof this.extendedMoments[$moment] == 'object' ) {
			return true;
		}
		return false;
	},

	addListener: function( $event, $func, $body, $target=null ) {
		this.extendedMoments[$event].push({body: $body, target: $target, do: $func, data: {}});
	},

	Listen: function(){
		AXELISTENERS = new AXELISTENERS();

		window.addEventListener('scroll', function(){
			if( !window.stillScrolling || Date.now()-window.stillScrolling >= 200 ){
				AXELISTENERS.inview();
				window.stillScrolling = Date.now();
			}
		});
	},

	extendedMoments: {'inview': []},
...
$XE.OPTIMIZE.registerCssProp("-tpx","perspective");
